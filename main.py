...
from scheduler.warmup_cosine_sched import WarmupCosineDecay
...
def get_optimizer(op_type, learning_rate):
    if op_type == 'sgd':
        ...
        return optimizers.Adam(learning_rate, clipnorm=6) #, clipnorm=6
    else:
...
def train(train_d, dev_d, test_d, kfold, dataset, neighbor_sample_size, embed_dim, n_depth, l2_weight, lr, optimizer_type,
          batch_size, aggregator_type, n_epoch, exp_name_simple, out_folder, callbacks_to_add=None, overwrite=True, model_type='kgcn'):
    ...
    config.dropout = 0.3
    config.num_heads = 3
    config.mlp_embed_factor = 2
    config.nonlin_func = tf.nn.relu
    ...
    # decay_steps_batch = int(len(train_d) / batch_size / 10)  
    # warmup_steps = 10
   
    # lr_schedule = WarmupCosineDecay(
    #         initial_learning_rate=lr,
    #         decay_steps=decay_steps_batch,
    #         warmup_steps=warmup_steps,
    #     )

    # config.optimizer.learning_rate = lr_schedule
    ...
    config.adj_entity = np.load(format_filename(os.path.join(PROCESSED_DATA_DIR, str_rep), ADJ_ENTITY_TEMPLATE,
                                                dataset=f'{dataset}_{neighbor_sample_size}'))
    config.adj_relation = np.load(format_filename(os.path.join(PROCESSED_DATA_DIR, str_rep), ADJ_RELATION_TEMPLATE,
                                                  dataset=f'{dataset}_{neighbor_sample_size}'))

    config.drug_feature = np.load(format_filename(os.path.join(PROCESSED_DATA_DIR, str_rep), DRUG_FEATURE_TEMPLATE, 
                                                  dataset=dataset, str_rep=str_rep, embed_dim=embed_dim),allow_pickle=True)
    ...
    config.callbacks_lr_decay = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_auc', 
                                                                     factor=0.3, patience=2, 
                                                                     min_lr=0.000001)

    config.exp_name = f'{model_type}_{dataset}_{out_folder}_neigh_{neighbor_sample_size}_embed_{embed_dim}_depth_' \
    ...
    model_save_path = os.path.join(config.checkpoint_dir, out_folder, '{}.tf'.format(config.exp_name))
    ...
    if not os.path.exists(model_save_path) or overwrite:
        ...
        model.fit(x_train=[train_data[:, :1], train_data[:, 1:2]], y_train=train_data[:, 2:3],
                  x_valid=[valid_data[:, :1], valid_data[:, 1:2]], y_valid=valid_data[:, 2:3], out_folder=out_folder)
    ...
    model.load_best_model(out_folder)
    ...
    outfile = f"{RESULT_DATA_DIR}/{out_folder}/{config.exp_name}.csv"
    ...
    write_log(format_filename(os.path.join(LOG_DIR, out_folder), 
                            PERFORMANCE_LOG.format(dataset, model_type)), 
